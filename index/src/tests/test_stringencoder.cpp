...
TEST_CASE("Test stringencoder", "[stringencoder]") {
    Matcher::StringEncoder se("a b c", "c b a");
    ...
    REQUIRE(oldwords.size() == 3);
    REQUIRE(newwords.size() == 3);
    ...
    // REQUIRE(se.decodeStream(oldstream) == {"a", "b", "c"});
    // REQUIRE(se.decodeStream(newstream) == "c b a");
